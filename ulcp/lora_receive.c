...
void on_recv(char *msg) {
  ...
  if (deserialized->header->packet_type == PONG &&
      strcmp(deserialized->header->transaction_uid,
  ...
  if (deserialized->header->packet_type == MSG) {
    ...
    size_t resize_len = strlen(this_lora->rx->recv_payloads_buf) +
                        strlen(deserialized->payload) + 1;
    this_lora->rx->recv_payloads_buf =
        (char *)realloc(this_lora->rx->recv_payloads_buf, resize_len);
  ...
  if (deserialized->header->packet_type == END) {
    if (strcmp(deserialized->header->transaction_uid,
      ...
      this_lora->rx->recv_transac_uid =
          calloc(TRANSACTION_UID_LENGTH + 1, sizeof(char));
      return;
    }
