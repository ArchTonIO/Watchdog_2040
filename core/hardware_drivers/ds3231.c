...
void ds3231_init(i2c_inst_t *i2c_port,
    uint8_t i2c_address,
    ds3231_rtc_t *rtc) {
  rtc->i2c_port = i2c_port;
  rtc->i2c_addr = i2c_address;
};
...
void ds3231_get_datetime(ds3231_datetime_t *dt, ds3231_rtc_t *rtc) {
  ...
  i2c_write_blocking(rtc->i2c_port,
      rtc->i2c_addr,
      &reg,
      1,
      true); // `true` to keep control
  i2c_read_blocking(rtc->i2c_port, rtc->i2c_addr, buffer, 7, false);
...
void ds3231_str_date(char *buf,
    uint8_t buf_size,
    const ds3231_datetime_t *dt) {
  snprintf(buf, buf_size, "%u-%02u-%02u", dt->year, dt->month, dt->day);
}
...
void ds3231_ctime(char *buf, uint8_t buf_size, const ds3231_datetime_t *dt) {
  ...
  snprintf(buf,
      buf_size,
      "%s %s %02u %02u:%02u:%02u %u",
      DS3231_WDAYS[dt->dotw - 1],
      DS3231_MONTHS[dt->month - 1],
      dt->day,
      dt->hour,
      dt->minutes,
      dt->seconds,
