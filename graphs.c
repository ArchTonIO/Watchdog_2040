...
/**
 * @brief Pushes a new value onto the graph, normalizing it and updating the buffer.
...
{
  uint8_t normalized = normalize(g, value, g->min_expected_value, g->max_expected_value);
...
void graph_update(graph *g)
{
  ...
  ssd1306_print(drivers->oled_screen, g->title, g->x_offset / 7, g->y_offset / 8 - 2, false);
  ssd1306_print(drivers->oled_screen, g->last_value_buf, g->x_offset / 7, g->y_offset / 8 - 1, false);
  point last_point;
...
uint8_t normalize(graph *g, int16_t val, int16_t min, int16_t max)
{
  ...
  if (val > max)
    val = max;
  return (uint8_t)((g->height - 1) - ((val - min) * (g->height - 1)) / (max - min));
}
