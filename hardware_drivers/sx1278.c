...
void spi_write_reg_multi_byte(
    sx1278 *radio, uint8_t reg, uint8_t *payload, size_t length);
uint8_t spi_read_reg_single_byte(sx1278 *radio, uint8_t reg);
void spi_read_reg_multi_byte(
    sx1278 *radio, uint8_t reg, uint8_t *buffer, size_t length);
...
sx1278 *sx1278_init(pin mosi,
  ...
  // printf("1 - REG_01_OP_MODE: %d\n", spi_read_reg_single_byte(new_radio,
  // REG_01_OP_MODE)); //! DEBUG SHIT
  sleep_ms(10);

  // check initialisation
  uint8_t reg_01_op_mode_content =
      spi_read_reg_single_byte(new_radio, REG_01_OP_MODE);
  ...
  // spi_read_reg_single_byte(new_radio, REG_0E_FIFO_TX_BASE_ADDR)); //! DEBUG
  // SHIT
  spi_write_reg_single_byte(new_radio, REG_0F_FIFO_RX_BASE_ADDR, 0x00);
  // printf("3 - REG_0F_FIFO_RX_BASE_ADDR: %d\n",
  // spi_read_reg_single_byte(new_radio, REG_0F_FIFO_RX_BASE_ADDR)); //! DEBUG
  // SHIT
...
void sx1278_set_mode_idle(sx1278 *radio) {
  if (radio->mode != MODE_STDBY) {
    ...
    // printf("4 - REG_01_OP_MODE: %d\n", spi_read_reg_single_byte(radio,
    // REG_01_OP_MODE)); //! DEBUG SHIT
  }
}
...
void irq_handler(uint gpio, uint32_t event_mask) {
  ...
  } else if (instance->mode == MODE_RXCONTINUOUS &&
             instance->irq_flags & RX_DONE) {
