...
uint8_t battery_get_percentage(battery *bat)
{
	...
	float v_bat = ((adc_read() * bat->vcc_measured_voltage) / bat->adc_max_value) * bat->voltage_divider_ratio;
...
{
	adc_select_input(bat->adc_channel);
	return ((adc_read() * bat->vcc_measured_voltage) / bat->adc_max_value) * bat->voltage_divider_ratio;
}
