...
char *gen_random_string(uint8_t length)
{
 ...
 char *stringify_packet(message *msg){
  char *buf = (char *)malloc(PACKET_MAX_SIZE);
  if (!buf) {
    return NULL;
  };
  snprintf(buf, PACKET_MAX_SIZE, "%04X%04X%s%02X%04X%s",
    msg->header->src_address,
    msg->header->dest_address,
    msg->header->transaction_uid,
  ...
  char *payload)
{
  ...
  head->payload_length = strlen(payload);
  msg->header = head;
  msg->payload = payload;
...
void send_start_packet(uint16_t dest_addr, char *transaction_uid){
  char *packet = build_packet(
    ...
    transaction_uid,
    "");
  sx1278_send_str(this_lora->radio, packet);
  free(packet);
