 ...
 *
 * @param file The path structure representing the file.
 * @return A pointer to a string list containing the file's contents, or NULL if an error occurred.
 */
...
/**
 * @brief Touches a file at the specified path, creating it if it does not exist.
 *
 * @param file The path structure representing the file.
...
{
  return sdcard_mkdir(drivers->sd_card, dir);
}
...
{
  return sdcard_rmdir(drivers->sd_card, dir);
}
 ...
 * @return A pointer to a string list containing the names of the files in the directory.
 */
 ...
 */
bool path_key_value_dump(path *file, char mode, const char *key, const char *value)
{
  return sdcard_write_key_value_to_file(drivers->sd_card, file, mode, key, value);
}

/**
 * @brief Retrieves the value associated with a key from a file at the specified path.
 *
 * @param file The path structure representing the file.
